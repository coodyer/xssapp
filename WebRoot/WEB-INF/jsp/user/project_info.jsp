								...
								<form action="" class="am-form" id="dataform" data-am-validator>
									...
									<fieldset>
										...
										<div class="am-form-group">
											<label for="doc-vld-name-2"><small>项目名称：</small></label> <input
												type="text" id="doc-vld-name-2" name="title"
												datatype="*1-48" errormsg="请输入正确的项目名" sucmsg="输入正确"
												nullmsg="请输入项目名" placeholder="输入项目名称（至少 1 个字符）"
												value="${project.title }" />
										</div>
										<div class="am-form-group">
										...
										<div class="am-form-group">
											...
											<textarea id="doc-vld-ta-2" name="filter">${project.filter }</textarea>
											<span>多个地址逗号分割</span>
										</div>
											...
											<c:if test="${!empty userModules }">
												...
												<p>
													<c:forEach items="${userModules }" var="module"
														varStatus="index">
														<input type="radio" name="module.id" datatype="n1-11"
															id="userModule${index.index}" errormsg="请选择模块"
															sucmsg="输入正确" nullmsg="请选择模块"
															<c:if test="${project.module.id==module.id }">checked="checked"</c:if>
															value="${module.id }" />
														<label style="color: #7A7D7E"
															for="userModule${index.index}"><small>${module.title }</small>
														</label>
											...
											<c:if test="${!empty sysModules }">
												...
												<p>
													<c:forEach items="${sysModules }" var="module"
														varStatus="index">
														<input type="radio" name="module.id" datatype="n1-11"
															id="sysModule${index.index}" errormsg="请选择模块"
															sucmsg="输入正确" nullmsg="请选择模块"
															<c:if test="${project.module.id==module.id }">checked="checked"</c:if>
															value="${module.id }" />
														<label style="color: #7A7D7E"
															for="sysModule${index.index}"><small>${module.title }</small>
														</label>
														<c:if test="${(index.index+1)%4==0 &&index.index!=0 }">
												</p>
												<p>
										...
										<div class="am-form-group">
											...
											<lable>&nbsp; &nbsp; </lable>
											<input type="checkbox" name="openMobile" id="openMobile"
										...
										<p>
											<button type="submit" name=""
												class="am-btn am-btn-success am-btn-xs">&nbsp;保
												存&nbsp;</button>
										</p>
									</fieldset>
								</form>
